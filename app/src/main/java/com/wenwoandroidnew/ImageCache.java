...
public class ImageCache {
	...
	private DiskLruCache1 mDiskLruCache;
			...
			if (mDiskLruCache == null || mDiskLruCache.isClosed()) {
				...
				if (diskCacheDir != null) {
					...
					if (getUsableSpace(diskCacheDir) > DEFAULT_DISK_CACHE_SIZE) {
						try {
							mDiskLruCache = DiskLruCache1.open(diskCacheDir, 1,
	...
	public void addBitmapToCache(String data, Bitmap value) {
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null) {
				...
				try {
					DiskLruCache1.Snapshot snapshot = mDiskLruCache.get(key);
					if (snapshot == null) {
						final DiskLruCache1.Editor editor = mDiskLruCache
								.edit(key);
						if (editor != null) {
    ...
    public Bitmap getBitmapFromDiskCache(String data) {
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    final DiskLruCache1.Snapshot snapshot = mDiskLruCache.get(key);
                    if (snapshot != null) {
